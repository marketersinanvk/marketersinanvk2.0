...
export default function Blog() {
  ...
  return (
    <div className="min-h-screen bg-transparent">
      <SEO 
        title="Elite Digital Marketing Insights | Best Digital Marketer in Palakkad, Kerala"
        description="Read the latest insights on AI marketing, Meta Ads strategies, and brand scaling from Muhammed Sinan VK. Best Digital Marketer in Palakkad, Kerala."
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 auto-rows-[450px]">
            {posts.map((post, i) => (
              <motion.article
                ...
                className={`${post.id === '1' ? "md:col-span-2 md:row-span-1" : "md:col-span-1 md:row-span-1"} glass-2 rounded-[40px] overflow-hidden group relative flex flex-col`}
              >
                ...
                <div className="relative z-10 p-10 mt-auto space-y-4">
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    <span className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-white/40 text-[8px] font-bold uppercase tracking-widest">
                      AI Marketing
                    </span>
                  </div>
                  
                  <h2 className="text-2xl md:text-3xl font-serif text-white leading-tight group-hover:text-neon-green transition-colors duration-500">
                    {post.title}
                  </h2>
                  ...
                  <div className="pt-6 flex items-center justify-between border-t border-white/5">
                    ...
                    <button className="w-10 h-10 glass-2 rounded-full flex items-center justify-center text-white group-hover:bg-neon-green group-hover:text-midnight transition-all duration-500">
                ...
                {/* Hover Glow Effect */}
                <div className="absolute inset-0 border border-neon-green/0 group-hover:border-neon-green/30 transition-all duration-700 pointer-events-none rounded-[40px]" />
                <div className="absolute -inset-px bg-neon-green/5 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none blur-xl" />
              </motion.article>
            ))}
          </div>
        </div>
